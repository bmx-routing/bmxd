...
Precompiled binaries for various linux systems can be found in the corresponding folder at:\\ 
...
%\item A number of promising parametrization sets have already been preconfigured in the code. The currently most performant and most general parametrization can be activated using the --bmx-defaults switch. 
...
\label{sec:algorithm}
The general path-detection algorithm works as follows: Every node propagates the knowledge about its existence over the mesh simply by flooding the network with originator messages.
%
Each originator message (OGM) can be uniquely identified by a sequence number and the IP address of the node that initiated the message.
%
...
In the first place, OGMs transmitted via a poor or saturated link will suffer from packetloss and delay.
%
Additionally, the propagation of OGMs over the mesh relies on intermediate nodes on the paths to re-broadcast a received OGM.
%
One general rule of the batman algorithm is: Each node only re-broadcasts the OGMs received via its currently best-ranking neighbor. 
%
...
%
By intentionally not accepting an OGM for the neighbor ranking, each node (along the propagation path of an OGM) also has the possibility to counteract on the propagation via specific links and influence the establishment of resulting routes in the mesh.
%
...
%
The computation of the complete end-to-end path between nodes is eliminated.
%
And the knowledge about the best end-to-end paths between nodes is divided to all nodes participating in the mesh. Each node perceives and maintains only the information about the best next hop towards all other nodes.
...
The main difference between the debug-level outputs of batmand-0.2 and batmand-experimental is in the output of debug level one.
...
\subsection{Reflecting the impact of asymmetric links}
...
As described in Section \ref{sec:batman-link-reflection}, the reflection of the RQ characteristics (along the paths in a mesh) is an inherent part of the flooding algorithm.
%
...
%
This approach incorporates a number of further attributes and advantages.
...
The following parameters exist to adapt batmand-experimental to fancy network environments. A more detailed documentation will follow...
...
\begin{verbatim}
       ...
       --t <value> : change default TTL of originator packets.
        ...
        /a : attached after an interface name
          to add IP address of this interface to the HNA list. Also
...
\begin{itemize}
 \item wlan interface must be given first
\end{itemize}
...
\item For a i386-based system precompiled binaries can be downloaded and installed at /usr/sbin/batmand with:
\begin{tiny}  \begin{verbatim}
